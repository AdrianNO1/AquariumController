...
6.0.3 uses the versioned per-device MQTT protocol while retaining
...
device's Wi-Fi, MQTT, and NTP settings, and flash firmware 6.0.3 once over USB.
...
Firmware 6.0.2 and newer start with physical outputs held at zero for at most
15 seconds.
...
so an update from it to a newer release cannot perform the symmetric wind-down
and fade-up sequence.
...
Firmware 6.0.3 also keeps the persisted local schedule running when network
...
4. Prepare the generic OTA artifact from the repository root:
   ...
   ```sh
   npm run firmware:release -- 6.0.3
   ```
...
`firmware/esp32/artifacts/ESP32Code-6.0.3.bin`; its exact size and SHA-256 are
