...
complete, including firmware 6.0.1, strict per-device JSON MQTT, bounded
...
3. Flash and identify firmware 6.0.1 on every production ESP32, then complete a
...
- Old/unexpected firmware is visible but excluded from actuator work until the
  operator flashes the current 6.0.1 release.
...
5. `firmware`: compile firmware 6.0.1 with the pinned Arduino toolchain.
...
2. Firmware 6.0.1 and focused transport/scheduler/compiler evidence pass locally.
