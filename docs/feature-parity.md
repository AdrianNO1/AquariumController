...
results below are explicitly historical evidence from the stated runs, not a
claim that final settled-tree validation after the latest hardening is complete
or that the local stack is currently running. The decisive evidence is:
...
- five Testcontainers tests against digest-pinned Mosquitto 2.0.22, covering
  ...
  cases;
- 17 retry-free Chromium tests against production-built assets, fresh SQLite
  databases, real Mosquitto, and independent MQTT fake ESP actors, covering
  routes, reload, responsive and axe checks, configuration CRUD, devices,
  overrides, logs, alerts, faults, and restarts;
- passing amd64 and ARM64 container, health, read-only/non-root, Compose, and
...
- ESP firmware 4.0.0, independent compatibility/failover tests, exact-version
  ...
  variables; and
- production-shaped legacy import plus SQLite integrity, backup, verification,
