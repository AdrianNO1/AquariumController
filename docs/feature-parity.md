...
| wire concurrency        | One FIFO lane and one response-waiting operation per ESP; four device lanes by default                                               | Priority, per-device ordering, concurrency, request-correlation, and dispatcher tests                 | A healthy ESP completes while another is delayed; chunk frames stay atomic and late/duplicate responses cannot settle a newer request          |
