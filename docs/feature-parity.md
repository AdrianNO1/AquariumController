...
replacement firmware 6.0.0 are compared explicitly.
...
All 17 retained rows are **Implemented** at repository level. The current 6.0.0
...
|   6 | Temporary/manual overrides                               | Legacy slider/manager and firmware `s`; override service/repository/routes/runtime/UI          | Server-authoritative start, extend, cancel, expire, restart initialization, reconcile, durable operations/targets, scheduler overlay, unscheduled outputs, exact 119999/120000-ms behavior, rollover safety, and outcome-unknown/no-retry behavior are covered. Controller writes renew a 120-second firmware overwrite lease, so local schedules resume only after Pi silence. The browser gets active state from a real fake-ESP response; broker/runtime tests cover refresh, controller stop, firmware failover, and controller/database restart recovery.                                                                                                                                                                                  | **Implemented.** Firmware 6.0.0 must still be flashed onto the production ESP32s.                                                                                |
...
|  10 | Schedule compilation, serialization, `syncTime`, DJB2    | Legacy compiler/manager and firmware schedule handler; domain/protocol/artifact repository     | Deterministic compact payload/hash, 4095/4096 boundary, persisted desired artifact, reported-hash no-op, protocol-v1 compatibility gate, mismatch/unknown outcomes, coalescing, restart, and independent fake SPIFFS/hash/evaluation tests exist. Compiled fallback schedules apply both the area multiplier and mapping-profile output multiplier, matching Pi-driven output after failover. Real-broker tests carry typed schedules in one MQTT publication, verify typed acceptance, hash announcement, evaluation, persistence/restart, and the separate time-sync command.                                                                                                                                                       | **Implemented.**                                                                                                                                                 |
...
|  13 | 120-second override/failover                             | Firmware 6.0.0, fake ESP, and override runtime                                                 | Firmware 6.0.0 retains rollover-safe overwrite expiry and cache invalidation after expiry, PWM reattachment, and schedule replacement, including a zero target. Controller-owned writes renew the lease; local schedules resume after Pi silence. Resolution reattachment rescales scheduled/current-overwrite caches and physical duty. If Pi and NTP are unavailable after reboot, valid persisted time intentionally permits local scheduling. Protocol gating blocks normal control of firmware outside the reviewed 6.x family.                                                                                                                                                                                                              | **Implemented in the repository.** Flashing and observing every deployed ESP32 remains external.                                                                 |
...
|  15 | MQTT serialization and response correlation              | Legacy manager and firmware callback; protocol/transport/fake                                  | MQTT 3.1.1 options, test-topic guards, versioned strict JSON, per-device topics, globally unique request IDs, batch-local indexes, max-three-per-target batching, an explicit 5,120-byte command ceiling, bounded priority-aware per-device lanes, and no blind retry exist. One operation may await a response per ESP and up to sixteen device lanes run by default. The short publication mutex is released before response waiting. Timeouts affect only one device; attributable malformed responses quarantine that device. A separately validated one-command legacy bridge exists only for operator-requested firmware-5 OTA and is excluded from update-all.                                               | **Implemented.** ESP MQTT is plaintext; production needs authenticated accounts, ACLs, and a listener restricted to the trusted LAN, or future physically validated TLS firmware. |
...
2. Legacy firmware accepted bare `clear` as an unauthenticated fleet-wide
   EEPROM erase. Firmware 6.0.0 deliberately removes that behavior; targeted
...
7. Legacy firmware versions `0`, `1`, and `2w` did not receive schedules. The
   replacement controls the reviewed firmware 6.x protocol only. Other versions
   remain visible as `firmware_unsupported` and are excluded from reconciliation
   and actuation. Firmware 5.x can receive only an explicit per-device OTA
   upgrade; older versions require USB.
...
- flash firmware 6.0.0 onto each physical ESP32, confirm each announcement is
...
A physical ESP outside the reviewed 6.x protocol remains visible in the
frontend with a `firmware_unsupported` error, but the controller does not send
it schedules or actuator operations. Firmware 5.x can be upgraded individually
through the website; update-all does not use that compatibility bridge.
