...
describe("manual override repository", () => {
  ...
  it("keeps unknown starts pending, never overlays them, and reconciles only after the safety window", async () => {
    const repository = await createRepository();
  ...
  it("recovers the unresolved child identifier after a crash between child and coordinator persistence", async () => {
    ...
    await requireDatabase()
      ...
      .values({
        ...
        deadline_at_ms: 15_002,
        completed_at_ms: 15_002,
    ...
    await expect(
      ...
      status: "outcome_unknown",
      result: {
        childOperationIds: ["child-crash-gap"],
        unknownChildOperationId: "child-crash-gap",
